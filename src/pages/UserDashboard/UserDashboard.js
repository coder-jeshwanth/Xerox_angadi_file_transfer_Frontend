...
const UserDashboard = () => {
    ...
    const fetchUploadedFiles = useCallback(async () => {
        try {
            const response = await fetch(
                `https://xerox-angadi-file-transfer-frontend.vercel.app/api/user/dashboard/${username}`
            );
            if (response.ok) {
    ...
    const handleFileUpload = async () => {
        ...
        try {
            const response = await fetch("https://xerox-angadi-file-transfer-frontend.vercel.app/api/user/upload", {
                method: "POST",
                body: formData,
            });
